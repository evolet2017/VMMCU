	            ...
	            retval.success(function (result) {
	                ...
	                if (value == 0) {
	                    ...
	                    bootbox.confirm("Save Entry?", function (result) {
	                        ...
	                        if (result) {
	                            //alert("true");
								var shareselect = $('#frmShareSelect').val(),
									checkamt = accounting.unformat($('#checkAmount').val()),
									checknumber = $('#checkNumber').val();
									
								var idlist =  ($('#checkidx').val()).split(',');
								var i = 0;
								
								while (i < idlist.length) {
								
									var checkid = (idlist[i].split(':'))[0],
										camt = (idlist[i].split(':'))[1];
										
										//alert(cid);
								    //alert(camt);
									    //var client = getXmlHttp();
									   // client.open("POST", MB.URLPoster(), false);
										var SQL = "UpdateMaster '" + checkid + "|" + shareselect + "','" + camt + "','7','" + checkid + "','" + checknumber + "','" + MB.getCookie('activeid') + "'";	                            										
								    //client.send(SQL);
										console.log(SQL);
										MB.push(SQL);
										
										//var ext = setTimeout(MB.push(SQL), 1000+(100*i));
										//ext = 0;
										//while (parseInt(ext) === 0)
										//{
										//	console.log(ext);
										//	console.log('timeout');
										//}
	                            ...
	                            //var SQL = "UpdateMaster '" + checkid + "|" + shareselect + "','" + checkamt + "','7','" + checkid + "','" + checknumber + "','" + MB.getCookie('activeid') + "'";	                            
	                            //console.log(SQL);
								
	                            //MB.push(SQL);
	                            var SQL = "Update BankChecks SET CheckDate='" + $('#checkDate').val() + "',TransDesc='Bank-Purchase',Particulars='" + $('#checkParticular').val() + "',IssuedTo='" + $('#clientName').val() + "',Amount=" + checkamt + ",TAG=1,v8RunDate=" + MB.getCookie('sysdate') + ",UserID='" + MB.getCookie('activeid') + "' WHERE GLACC+CONVERT(VARCHAR,CHECKID)=dbo.TRIM('" + $('#frmShareSelect').val() + $('#checkNumber').val() + "') ";
			...
			var ids = ""; 
	            $.each(JSON.parse(data), function (id, value) {
	                ...
	                if(value.tag != "1"){
	                    var tr = "<tr>";
			    idx += 1;
			    ids = "idcheck"+idx.toString();
			    tr += "<td><input value='"+value.id+":"+value.pur_cost+"' type='checkbox' class='minimal' name='idcheck' id='"+ids+"' onclick='pushpo(\""+ids+"\","+value.id+","+value.pur_cost+")'>	</td>";
	                   ...
	                   // tr += "<td>" + value.glacc + "</td>";
	                    //tr += "<td id='" + value.id + "' onclick='BankPurchase.initValue("+value.id+");'><a class='label label-warning' data-toggle='modal' data-target='#swmanager'>" + value.tag + "</a></td>";
						tr += "<td></td>";
						tr += "<td></td></tr>";
