...
var BankAsset = {
    ...
    bankassetupdate: function () {
        ...
        bootbox.confirm("Update Record?", function (result) {
            if (result) {
                ...
                sql += "po_salvagevalue=" + $('#pur_salvagevalue').val() + ",";
	...
	bankassetsave : function() {	
			...
			bootbox.confirm("Post Entry?", function(result){
				if (result) {
				    var sql = "INSERT INTO BankAsset (asset_code,asset_desc,asset_expire,asset_serial,asset_po,pur_desc,pur_date,pur_cost,pur_isnew,GLACC,userid,pur_salvagevalue,pur_deprevalue,pur_lifeinyears,pur_annualexpense,DebitGLAcct,CreditGLAcct,VendorLink) VALUES ";
						sql += "('"+$('#assetnum').val()+"','";
						sql += $('#asset_desc').val()+"','";
						...
						sql += $('#SelectAccu').val() + "','";
						sql += $('#SelectExp').val() + "','" + $('#po_vendorlist').val() + "')";
	...
	bankfixasset: function () {
	    ...
	    retv1.success(function (msg) {
	        $.each(JSON.parse(msg), function (id, value) {
	            ...
	            tr += "<td class='no-print'><button type='button' class='btn btn-success btn-xs' data-toggle='on' onclick=editasset('"+value.asset_code+"')>Edit</button></td>";
