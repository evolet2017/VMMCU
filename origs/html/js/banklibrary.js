var BankDeposit = {
	...
	postcash: function (s) {
	    ...
	    //console.log("amt-> " + $('#frmBankDepoAmt').val());
	    //if ($('#frmBankDepoAmt').val() === "") {
	    //    bootbox.alert("Invalid amount!!");
	    //    return
	    //}
	...
	postcheck : function() {
              ...
              MB.push("INSERT INTO BankCheckTrn (v8RunDate,TrnType,Amount,GLCode,CheckNumber,IssuingBank,IssuedBy,Tag,Particulars,CheckDate) VALUES ('" + MB.getCookie("sysdate") + "',2," + accounting.unformat($('#chkAmount').val()) + ",'" + $('#frmSelect').val() + "'," + $('#chkNumber').val() + ",'" + $('#chkIssuingBank').val() + "','" + $('#chkIssuedBy').val() + "',1,'" + $('#chkParticular').val() + "','" + $('#chkDate').val() + "')");
	...
	bankcashinfoload: function () {
	    var sv = "SELECT acc account,ISNULL(fname+' '+lname,'') fullname,trnamt/100 amount FROM dashboard WHERE trntype IN ('01','93') and trnnonc=0 and trndate=" + MB.getCookie("sysdate");
	    console.log(sv);
	    $.post(MB.URLPoster(),  //
		{
		    SQLStatement: sv
